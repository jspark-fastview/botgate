-- Spring Boot DataSource initialization (auto-run on startup).
-- spring.sql.init.continue-on-error=true 라 ALTER 가 실패해도 (이미 컬럼 존재) 무시.
--
-- 주의: token-api/src/db/schema.js 와 동기화 유지. JS 측이 canonical source.
-- 변경 시 양쪽 모두 반영.
...
-- ── channels (모든 컬럼 포함 — Tollbit-style 외부통합 컬럼까지) ──────
