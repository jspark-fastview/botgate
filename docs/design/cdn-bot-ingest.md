...
adapter/
  core/         공통 — HTTP receiver, CDN별 auth(secret/HMAC), schema validate,
                normalize pipeline, source labeling, dedup, Loki push + access_logs write
  plugins/
...
- [x] **Loki ingest = stdout→alloy** (기존 패턴). adapter 는 canonical JSON 을 stdout 에 찍고,
      alloy 가 긁어 Loki push. 근거: alloy 가 parse/label/버퍼/재시도/백프레셔 처리 →
      adapter 는 변환에만 집중, Loki endpoint 변경에 무영향 (loki-gateway 전환 시 alloy 만 수정한 선례).
      → **필요 작업**: alloy config 에 app=bot-ingest stdout 도 JSON parse + `source` label promote 추가.
      → 추후 초당 대량 burst 로 alloy 가 못 따라가면 Loki 직접 push 전환 검토.
