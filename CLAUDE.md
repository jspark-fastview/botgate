...
## 🚨 0. 작업 범위 임의 축소 / 확장 금지 (최상위)
**축소도 확장도 자기 판단 X. 사용자 의도 정확히 매칭.**

### 축소 금지
"오늘은", "일단", "지금은" 같은 말로 작업 임의 축소 X. 사용자가 명시한 의도 (예: "다 정리하자", "전부 자동화") 가 있으면 그 끝까지 완수.

### 확장 금지
사용자가 시키지 않은 인접 작업 임의 추가 X. 예: "K8s Ingress 자동화" 요청에 Cloudflare DNS 자동화까지 끼워넣지 말 것.

### 모호하면 물어보기
- 시간 절약 / 안전 우려로 부분만 하려면 사용자에게 명시적으로 확인.
- "이거 까지 자동화할까?" / "전체 작업 N단계, 어느 단계까지 진행할까?" 라고 먼저 묻기.
- 사용자 결정 없이 "1단계만" "오늘은 여기까지" 또는 "겸사겸사 X도 추가" 같은 자체 판단 금지.
